...
>>>>>>> Update
  ...
  _ensureConnection() {
    ...
    if (this._rpcConnection == null) {
      ...
      processStream.switchMap(proc => (0, _process().getOutputStream)(proc, {
        /* TODO(T17353599) */
        isExitError: () => false
      ...
      const connection = this._rpcConnection = processStream.take(1).toPromise().then(proc => {
        ...
        return new (_RpcConnection().RpcConnection)('client', this._serviceRegistry, new (_StreamTransport().StreamTransport)(proc.stdin, proc.stdout, this._messageLogger));
      });
  ...
  _onProcessMessage(message) {
    switch (message.kind) {
      ...
      case 'exit':
        logger.error(`${this._name} - exited with ${(0, _process().exitEventToMessage)(message)}`);
