...
function _ObjectRegistry() {
  const data = require("./ObjectRegistry");
...
class Subscription {
  ...
  error(error) {
    try {
      this._observer.error((0, _messages().decodeError)(this._message, error));
    } catch (e) {
  ...
  next(data, bytes) {
    try {
      this._observer.next(data); // TODO: consider implementing a rate limit
...
class Call {
  ...
  reject(error) {
    if (!this._complete) {
      ...
      this._reject((0, _messages().decodeError)(this._message, error));
    }
  }
...
>>>>>>> Update
  ...
  static createRemote(transport, predefinedTypes, services, options = {}, protocol = _config().SERVICE_FRAMEWORK3_PROTOCOL, connectionId = null, protocolLogger = null) {
    return new RpcConnection('client', new (_ServiceRegistry().ServiceRegistry)(predefinedTypes, services, protocol, {
      lazy: true
    }), transport, options, connectionId, protocolLogger);
  } // Creates a client side connection to a server on the same machine.


  static createLocal(transport, predefinedTypes, services, protocol = _config().SERVICE_FRAMEWORK3_PROTOCOL, connectionId = null, protocolLogger = null) {
    return new RpcConnection('client', // We can afford to be lazy when creating the RPC client.
    // Client code always explicitly loads services by name!
    new (_ServiceRegistry().ServiceRegistry)(predefinedTypes, services, protocol, {
      lazy: true
    }), transport, {}, connectionId, protocolLogger);
  }
  ...
  callRemoteFunction(functionName, returnType, args) {
    return this._sendMessageAndListenForResult((0, _messages().createCallMessage)(this._getProtocol(), functionName, this._generateRequestId(), args), returnType, `Calling function ${functionName}`);
  }
  /**
  ...
  callRemoteMethod(objectId, methodName, returnType, args) {
    return this._sendMessageAndListenForResult((0, _messages().createCallObjectMessage)(this._getProtocol(), methodName, objectId, this._generateRequestId(), args), returnType, `Calling remote method ${methodName}`);
  }
  /**
  ...
  async disposeRemoteObject(object) {
    ...
    if (objectId == null) {
      ...
      return this._sendMessageAndListenForResult((0, _messages().createDisposeMessage)(this._getProtocol(), this._generateRequestId(), objectId), 'promise', `Disposing object ${objectId}`);
    }
  }
  /**
  ...
  _sendMessageAndListenForResult(message, returnType, timeoutMessage) {
    switch (returnType) {
      ...
      case 'promise':
        ...
        const {
          trackSampleRate
        } = this._options; // flowlint-next-line sketchy-null-number:off

        if (trackSampleRate && Math.random() * trackSampleRate <= 1) {
          return (0, _nuclideAnalytics().trackTiming)(trackingIdOfMessageAndNetwork(this._objectRegistry, message), () => promise);
        }
      ...
      case 'observable':
        {
          ...
          const sendUnsubscribe = () => {
            if (!this._transport.isClosed()) {
              this._transport.send(JSON.stringify((0, _messages().createUnsubscribeMessage)(this._getProtocol(), id)));
            }
          };
...
>>>>>>> Update
            ...
            sendSubscribe(); // Observable dispose function, which is called on subscription dispose, on stream
  ...
  _returnPromise(id, candidate, type) {
    let returnVal = candidate; // Ensure that the return value is a promise.
    ...
    returnVal.then(result => {
      this._transport.send(JSON.stringify((0, _messages().createPromiseMessage)(this._getProtocol(), id, this._generateResponseId(), this.marshal(result, type))));
    }, error => {
      this._transport.send(JSON.stringify((0, _messages().createErrorResponseMessage)(this._getProtocol(), id, this._generateResponseId(), error)));
    });
  }
...
>>>>>>> Update
    ...
    .subscribe(data => {
      this._transport.send(JSON.stringify((0, _messages().createNextMessage)(this._getProtocol(), id, this._generateResponseId(), data)));
    }, error => {
      this._transport.send(JSON.stringify((0, _messages().createObserveErrorMessage)(this._getProtocol(), id, this._generateResponseId(), error)));

      this._objectRegistry.removeSubscription(id);
    }, completed => {
      this._transport.send(JSON.stringify((0, _messages().createCompleteMessage)(this._getProtocol(), id, this._generateResponseId())));
  ...
  _callMethod(id, call) {
    ...
    const {
      definition
    } = this._getClassDefinition(interfaceName);
  ...
  _handleResponseMessage(message, rawMessage) {
    const id = message.id; // Keep track of the request message for logging

    let requestMessage = null; // Here's the main message handler ...
    ...
    if (responseId !== this._lastResponseId + 1 && this._lastResponseId !== -1 && requestMessage != null && this._options.trackSampleRate != null) {
      ...
      (0, _nuclideAnalytics().track)('response-message-id-mismatch', {
        eventName,
        responseId,
  ...
  _handleRequestMessage(message) {
    ...
    if (id !== this._lastRequestId + 1 && this._lastRequestId !== -1 && // We're excluding Unsubscribe messages since they reuse the IDs from
    ...
    message.type !== 'unsubscribe') {
      ...
      (0, _nuclideAnalytics().track)('message-id-mismatch', {
        eventName,
        id,
    ...
    try {
      switch (message.type) {
        ...
        case 'dispose':
          this._returnPromise(id, this._objectRegistry.disposeObject(message.objectId), _builtinTypes().voidType);
      ...
      this._transport.send(JSON.stringify((0, _messages().createErrorResponseMessage)(this._getProtocol(), id, this._generateResponseId(), e)));
    }
  }
  ...
  _trackLargeResponse(message, size) {
    ...
    const eventName = trackingIdOfMessage(this._objectRegistry, message);
    const args = message.args != null ? (0, _string().shorten)(JSON.stringify(message.args), 100, '...') : '';
    logger.warn(`${eventName}: Large response of size ${size}. Args:`, args);
    (0, _nuclideAnalytics().track)('large-rpc-response', {
      eventName,
      size,
      args
    });
