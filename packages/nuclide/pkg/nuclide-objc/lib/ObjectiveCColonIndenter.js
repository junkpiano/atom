 ...
 * @format
 */
const GRAMMARS = ['source.objc', 'source.objcpp']; // The indentation amount depends on previous lines. If the user types a colon outside of a method
...
class ObjectiveCColonIndenter {
  enable() {
    ...
    subscriptions.add((0, _observeLanguageTextEditors().default)(GRAMMARS, textEditor => this._enableInTextEditor(textEditor), textEditor => this._disableInTextEditor(textEditor)));
  }
  ...
  _enableInTextEditor(textEditor) {
    this._insertTextSubscriptionsMap.set(textEditor, textEditor.onDidInsertText(event => {
      (0, _nuclideAnalytics().trackTiming)('objc:indent-colon', () => {
        const {
          range,
          text
        } = event; // Ignore the inserted text if the user is typing in a string or comment.
        //
        ...
        const colonColumn = ObjectiveCColonIndenter.getIndentedColonColumn(buffer, currentColonPosition); // flowlint-next-line sketchy-null-number:off
        ...
        const unindentedLine = line.trimLeft(); // 2. Calculate the amount of indentation the line should end up with.
        ...
        const totalIndentAmount = unindentedCurrentColonColumn >= colonColumn ? 0 : colonColumn - unindentedCurrentColonColumn; // 3. Replace the current line with the properly-indented line.

        textEditor.setTextInBufferRange(buffer.rangeForRow(currentColonPosition.row,
        /* includeNewline */
        false), ' '.repeat(totalIndentAmount) + unindentedLine); // Move the cursor to right after the inserted colon.
  ...
  static getIndentedColonColumn(buffer, startPosition) {
    ...
    buffer.backwardsScanInRange( // Only stop at the key characters: `:[]+-`.
    /:|\[|]|\+|-/g, _atom.Range.fromObject([startPosition.translate([-NUMBER_OF_PREVIOUS_LINES_TO_SEARCH_FOR_COLONS, 0]), startPosition.translate([0, -1])]), ({
      match,
      matchText,
      range,
      stop
    }) => {
      const position = range.start; // If we find a key character on the starting line, then the user is
      // typing a single-line method (it doesn't need to be indented).

      const isSingleLineMethod = position.row === startPosition.row; // `+` or `-` means we've reached the start of a method declaration.
