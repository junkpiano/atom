...
const VALID_NUX_POSITIONS = new Set(['top', 'bottom', 'left', 'right', 'auto']); // The maximum number of times the NuxView will attempt to attach to the DOM.
...
class NuxView {
  ...
  _createNux(creationAttempt = 1) {
    if (creationAttempt > ATTACHMENT_ATTEMPT_THRESHOLD) {
      this._onNuxComplete(false); // An error is logged and tracked instead of simply throwing an error since this function
    ...
    const debouncedWindowResizeListener = (0, _debounce().default)(this._handleWindowResize.bind(this), RESIZE_EVENT_DEBOUNCE_DURATION, false);
    window.addEventListener('resize', debouncedWindowResizeListener); // Destroy the NUX if the element it is bound to is no longer visible.
    ...
    const boundClickListener = this._handleDisposableClick.bind(this, true
    /* continue to the next NUX in the NuxTour */
    );
  ...
  _createDisposableTooltip() {
    ...
    const LINK_DISABLED = 'nuclide-nux-link-disabled'; // Let the link to the next NuxView be enabled iff
    //  a) it is not the last NuxView in the tour AND
    //  b) there is no condition for completion

    const nextLinkStyle = !this._finalNuxInTour && this._completePredicate == null ? LINK_ENABLED : LINK_DISABLED; // Additionally, the `Next` button may be disabled if an action must be completed.
    ...
    if (nextLinkStyle === LINK_ENABLED) {
      const nextElementClickListener = this._handleDisposableClick.bind(this, true
      /* continue to the next NUX in the tour */
      );
      ...
      this._disposables.add(new (_UniversalDisposable().default)(() => nextElement.removeEventListener('click', nextElementClickListener)));
    ...
    const dismissElementClickListener = !this._finalNuxInTour ? this._handleDisposableClick.bind(this, false
    /* skip to the end of the tour */
    ) : this._handleDisposableClick.bind(this, true
    /* continue to the next NUX in the tour */
    );
    ...
    this._disposables.add(new (_UniversalDisposable().default)(() => dismissElement.removeEventListener('click', dismissElementClickListener)));
  }
  ...
  _track(message, error) {
    (0, _nuclideAnalytics().track)('nux-view-action', {
      tourId: this._tourId,
      message: `${message}`,
