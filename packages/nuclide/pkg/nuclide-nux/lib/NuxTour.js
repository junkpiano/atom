...
class NuxTour {
  ...
  forceEnd(shouldMarkAsCompleted = false) {
    ...
    this._nuxList[this._currentStep].dispose(); // Skip remaining NUXes. No disposal is needed since they are lazily instantiated.
  ...
  _track(completed = false, error) {
    (0, _nuclideAnalytics().track)('nux-tour-action', {
      tourId: this._id,
      tourName: this._name,
