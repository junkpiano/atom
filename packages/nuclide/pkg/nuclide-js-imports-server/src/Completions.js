...
class Completions {
  ...
  provideCompletions(textDocumentPosition, nuclideFormattedUri) {
    const {
      position,
      textDocument
    } = textDocumentPosition; // TODO(seansegal): Handle imports broken up on multiple lines.

    const document = this.documents.get(textDocument.uri);
    const line = document.buffer.lineForRow(position.line);

    if (positionIsAtLineEnd(line, position) && // Check if line starts with "import" (or "const") before matching all regexes.
...
function provideFullImportCompletions(importInformation, importsFormatter, autoImportsManager, nuclideFormattedUri, line, lineNum) {
  ...
  const exportsIndex = autoImportsManager.exportsManager.getExportsIndex(); // 1) Find all IDs that fuzzily match the given string.
  ...
  return matchingIds.reduce((results, id) => {
    ...
    const exportsForId = exportsIndex.getExportsFromId(id); // 3) Filter and sort the exports, and add them to the return list of completions.
...
function provideImportFileCompletions(importInformation, importsFormatter, autoImportsManager, nuclideFormattedUri, line, lineNum) {
  const {
    ids,
    importType
  } = importInformation; // Intersect all exports for `ids` and then filter/sort the result.
...
function findCommonSuggestions(autoImportsManager, ids, importType) {
  ...
  const commonUris = suggestionsForEachId.reduce((aggregated, suggestionForId) => {
    return (0, _collection().setIntersect)(aggregated, new Set(suggestionForId.map(s => s.uri)));
...
function positionIsAtLineEnd(line, position) {
  ...
  const remainder = line.substr(position.character).trim(); // Still provide autocomplete if there is trailing whitespace.
