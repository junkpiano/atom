...
function HgService() {
  const data = _interopRequireWildcard(require("../lib/HgService"));
...
function hgUtils() {
  const data = _interopRequireWildcard(require("../lib/hg-utils"));
...
describe('HgService', () => {
  ...
  const PATH_1 = _nuclideUri().default.join(TEST_WORKING_DIRECTORY, 'test1.js');
  ...
  describe('::createBookmark', () => {
    ...
    it('calls the appropriate `hg` command to add', async () => {
      ...
      expect(hgUtils().hgAsyncExecute).toHaveBeenCalledWith(['bookmark', BOOKMARK_NAME], {
    ...
    it('calls the appropriate `hg` command to add with base revision', async () => {
      ...
      expect(hgUtils().hgAsyncExecute).toHaveBeenCalledWith(['bookmark', '--rev', BASE_REVISION, BOOKMARK_NAME], {
        cwd: TEST_WORKING_DIRECTORY
      });
    });
  });
  describe('::deleteBookmark', () => {
    ...
    it('calls the appropriate `hg` command to delete', async () => {
      ...
      expect(hgUtils().hgAsyncExecute).toHaveBeenCalledWith(['bookmarks', '--delete', BOOKMARK_NAME], {
        cwd: TEST_WORKING_DIRECTORY
      });
    });
  });
...
>>>>>>> Update
    ...
    it('fetches the unified diff output for the given path.', async () => {
      ...
      jest.spyOn(hgUtils(), 'hgAsyncExecute').mockImplementation((args, options) => {
        expect(args.pop()).toBe(PATH_1);
        return Promise.resolve({
          stdout: mockHgDiffOutput
        });
      });
  ...
  describe('::getConfigValueAsync', () => {
    it('calls `hg config` with the passed key', async () => {
      jest.spyOn(hgUtils(), 'hgAsyncExecute').mockImplementation((args, options) => {
        expect(args).toEqual(['config', 'committemplate.emptymsg']); // Return the Object shape expected by `HgServiceBase`.
  ...
  describe('::commit|amend', () => {
    ...
    beforeEach(() => {
      expectedArgs = null;
      committedToHg = false;
      jest.spyOn(hgUtils(), 'hgObserveExecution').mockImplementation((_args, options) => {
        const args = _args;
        expect(expectedArgs).not.toBeNull(); // eslint-disable-next-line eqeqeq
...
>>>>>>> Update
    ...
    describe('::amend', () => {
      ...
      it('can amend with --rebase & a commit message', async () => {
        ...
        await hgService.amend(TEST_WORKING_DIRECTORY, commitMessage, _hgConstants().AmendMode.REBASE).refCount().toArray().toPromise(); // 'Looks like commit did not happen'

        expect(committedToHg).toBeTruthy();
      });
      it('can amend with --fixup', async () => {
        expectedArgs = ['amend', '--fixup'];
        await hgService.amend(TEST_WORKING_DIRECTORY, null, _hgConstants().AmendMode.FIXUP).refCount().toArray().toPromise();
        expect(committedToHg).toBeTruthy();
      });
    });
  });
...
>>>>>>> Update
      ...
      expect(conflicts[0].status).toBe(_hgConstants().MergeConflictStatus.BOTH_CHANGED);
      expect(conflicts[1].status).toBe(_hgConstants().MergeConflictStatus.DELETED_IN_OURS);
    });
  });
...
>>>>>>> Update
    ...
    it('calls hg resolve', async () => {
      await hgService.markConflictedFile(TEST_WORKING_DIRECTORY, PATH_1,
      /* resolved */
      true).refCount().toArray().toPromise();
      expect(hgUtils().hgObserveExecution).toHaveBeenCalledWith(['resolve', '-m', PATH_1], {
        cwd: TEST_WORKING_DIRECTORY
      });
    });
  });
  describe('::_checkConflictChange', () => {
    ...
    beforeEach(() => {
      hgRepoSubscriptions = new (HgService().HgRepositorySubscriptions)(TEST_WORKING_DIRECTORY);
...
>>>>>>> Update
      ...
      subject.next(m1);
      subject.next(m2);
      repoSubscriptions.dispose(); // after disposing, we shouldn't see any more emitted events

      subject.next(m1);
      subject.next(m2); // await goes through even though we never called subject.complete()
