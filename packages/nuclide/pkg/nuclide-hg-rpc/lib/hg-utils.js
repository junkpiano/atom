...
function _nuclideRemoteAtomRpc() {
  const data = require("../../nuclide-remote-atom-rpc");

  _nuclideRemoteAtomRpc = function () {
    return data;
  };
...
const COMMIT_MESSAGE_STRIP_LINE = /^HG:.*(\n|$)/gm; // Avoid spamming the hg blackbox with read-only hg commands.

const EXCLUDE_FROM_HG_BLACKBOX_COMMANDS = new Set([// 'bookmarks' is technically another read-only command, but the possible
...
async function hgAsyncExecute(args_, options_) {
  const {
    ...
    args,
    options
  } = await getHgExecParams(args_, options_);

  try {
    return await (0, _process().runCommandDetailed)(command, args, options).toPromise();
  } catch (err) {
...
>>>>>>> Update
    ...
    return (0, _process().observeProcess)(command, args, Object.assign({}, options, {
...
>>>>>>> Update
    ...
    args,
    options
  }) => (0, _process().runCommand)(command, args, Object.assign({}, options, {
    killTreeWhenDone: true
  })));
}
...
function logAndThrowHgError(args, options, err) {
  if (err instanceof _process().ProcessExitError) {
    (0, _log4js().getLogger)('nuclide-hg-rpc').error(`Error executing hg command: ${JSON.stringify(args)}\n` + `stderr: ${err.stderr}\nstdout: ${String(err.stdout)}\n` + `options: ${JSON.stringify(options)}`);
    const {
      stdout,
      stderr,
      exitCode
...
async function getHgExecParams(args_, options_) {
  ...
  if (process.platform !== 'win32') {
    const pathToSSHConfig = _nuclideUri().default.expandHomeDir('~/.atom/scm_ssh.sh');

    const doesSSHConfigExist = await _fsPromise().default.exists(pathToSSHConfig);
    ...
    args.push('--config', `ui.ssh=${sshCommand}`, // enable the progressfile extension
  ...
  if (!options.NO_HGPLAIN) {
    ...
    options.env.HGPLAIN = 1; // Make an exception for plain mode so the progress file gets written
...
async function getInteractiveCommitEditorConfig() {
  const connectionDetails = await (0, _nuclideRemoteAtomRpc().getConnectionDetails)();

  if (connectionDetails == null) {
    (0, _log4js().getLogger)('nuclide-hg-rpc').error('CommandServer not initialized!');
    return null;
