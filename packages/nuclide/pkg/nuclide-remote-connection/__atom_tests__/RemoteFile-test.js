...
describe('RemoteFile', () => {
  ...
  describe('getRealPath() & getRealPathSync()', () => {
    ...
    beforeEach(() => {
      const tempDir = _temp().default.mkdirSync('realpath_test');

      filePath = _nuclideUri().default.join(tempDir, 'file.txt');
    ...
    it('gets realpath of a file', async () => {
      const file = new (_RemoteFile().RemoteFile)(_connection_mock().default, symlinkedFilePath);
  ...
  describe('delete()', () => {
    ...
    it('deletes the existing file', async () => {
      const filePath = _nuclideUri().default.join(tempDir, 'file_to_delete');

      _fs.default.writeFileSync(filePath, '');

      const file = new (_RemoteFile().RemoteFile)(_connection_mock().default, filePath);
  ...
  describe('RemoteFile::onDidDelete()', () => {
    let tempDir;
    beforeEach(() => {
      tempDir = _temp().default.mkdirSync('on_did_create_and_delete_test');
    });
    it('calls on delete', async () => {
      ...
      file.delete();
      await (0, _waits_for().default)(() => callbackSpy.mock.calls.length > 0);
  ...
  describe('RemoteDirectory::isSymbolicLink()', () => {
    ...
    it('verifies symlink', () => {
      const targetFilePath = _nuclideUri().default.join(tempDir, 'target');

      const symLinkedFilePath = _nuclideUri().default.join(tempDir, 'linked');
      ...
      const file = new (_RemoteFile().RemoteFile)(_connection_mock().default, `nuclide://host13${symLinkedFilePath}`, true);
      const symlink = file.isSymbolicLink();
      expect(symlink).toBe(true);
    });
    it('verifies non-symlink', () => {
      const notLinkedFilePath = _nuclideUri().default.join(tempDir, 'not_linked');

      _fs.default.writeFileSync(notLinkedFilePath, '');

      expect(_fs.default.lstatSync(notLinkedFilePath).isSymbolicLink()).toBe(false);
      const file = new (_RemoteFile().RemoteFile)(_connection_mock().default, `nuclide://host13${notLinkedFilePath}`, false);
      const symlink = file.isSymbolicLink();
  ...
  describe('copy()', () => {
    let tempDir;
    beforeEach(() => {
      tempDir = _temp().default.mkdirSync('copy_test');
    ...
    it('copying existing files', async () => {
      const filePath = _nuclideUri().default.join(tempDir, 'file_to_copy');

      const fileContents = 'copy me!';

      _fs.default.writeFileSync(filePath, fileContents);

      const newFilePath = _nuclideUri().default.join(tempDir, 'copied_file');
      ...
      const newFile = new (_RemoteFile().RemoteFile)(_connection_mock().default, newFilePath);
  ...
  xdescribe('RemoteFile watchFile integration', () => {
    ...
    beforeEach(async () => {
      ...
      tempDir = _temp().default.mkdirSync('on_did_change_test');
      filePath = _nuclideUri().default.join(tempDir, 'file.txt');
      file = new (_RemoteFile().RemoteFile)(_connection_mock().default, filePath);

      _fs.default.writeFileSync(filePath, 'sample contents'); // Ask watchman to watch the directory.


      await (() => _connection_mock().default.getFsService().watchDirectoryRecursive(tempDir))(); // wait for the watchman to settle on the created directory and file.
    ...
    afterEach(async () => {
      ...
      await _connection_mock().default.getFsService().unwatchDirectoryRecursive(tempDir);
    });
    ...
    describe('when the file is deleted, but not through the client', () => {
      it('notifies ::onDidDelete observers', () => {
        ...
        (0, _waits_for().default)(() => deletionHandler.mock.calls.length > 0);
    ...
    xdescribe('when the file is renamed', () => {
      it('notifies ::onDidRename observers', () => {
        ...
        (0, _waits_for().default)(() => renameHandler.mock.calls.length > 0);
        runs(() => {
          expect(renameHandler.mock.calls.length).toBe(1);
          expect(renameHandler.mock.calls[0][0]).toBe(_nuclideUri().default.basename(filePath + '_moved'));
        });
      });
    });
    describe('when a watch handling error happens', () => {
      it('notifies ::onWillThrowWatchError observers', async () => {
        const notExistingFile = new (_RemoteFile().RemoteFile)(_connection_mock().default, _nuclideUri().default.join(tempDir, 'no_existing.txt'));
        let skippedError;
        let handleError;
        ...
        (0, _waits_for().default)(() => skippedError);
        runs(() => {
  ...
  describe('::create()', () => {
    let tempDir;
    beforeEach(() => {
      tempDir = _temp().default.mkdirSync('create_test');
    });
    it('returns true when file creation is successful', async () => {
      const filePath = _nuclideUri().default.join(tempDir, 'create.txt');

      const remoteFile = new (_RemoteFile().RemoteFile)(_connection_mock().default, filePath);
  ...
  describe('::exists()', () => {
    ...
    it('exists resolves to true when the file exists', async () => {
      const filePath = _nuclideUri().default.join(tempDir, 'file.txt');

      _fs.default.writeFileSync(filePath, 'sample contents');

      const existingFile = new (_RemoteFile().RemoteFile)(_connection_mock().default, filePath);
  ...
  describe('RemoteFile::getParent()', () => {
    ...
    it('gets the parent directory for a file in a root directory', () => {
      ...
      const file = new (_RemoteFile().RemoteFile)(server, filePath);
...
>>>>>>> Update
      ...
      file.onDidChange(changeHandler);
      file.onDidDelete(deletionHandler); // The file tree sets the path before doing the rename.

      file.setPath('test2'); // Simulate a Watchman rename (delete + change)
  ...
  describe('RemoteFile::createReadStream()', () => {
    it('is able to read file contents', async () => {
      const tempDir = _temp().default.mkdirSync('stream_test');
      ...
      const data = await (0, _stream().observeStream)(readStream).toArray().toPromise();
      expect(data).toEqual(['test1234']);
    });
    it('handles errors', async () => {
      const file = new (_RemoteFile().RemoteFile)(_connection_mock().default, 'test');
      const readStream = file.createReadStream();
      const data = await (0, _stream().observeStream)(readStream).toArray().toPromise().catch(e => e);
  ...
  describe('RemoteFile::createWriteStream()', () => {
    ...
    it('handles errors', async () => {
      const file = new (_RemoteFile().RemoteFile)(_connection_mock().default, 'a/');
      const writeStream = file.createWriteStream();
      writeStream.write('test1234');
      let error = null;
