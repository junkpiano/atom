  ...
  constructor(host, port, heartbeat) {
    ...
    const onHeartbeatError = error => {
      const {
        ...
        message,
        originalCode
      } = error; // Don't keep tracking consecutive NETWORK_AWAY events:
...
>>>>>>> Update
    ...
    this._subscription = new (_UniversalDisposable().default)(heartbeat.onHeartbeat(onHeartbeat), heartbeat.onHeartbeatError(onHeartbeatError));
