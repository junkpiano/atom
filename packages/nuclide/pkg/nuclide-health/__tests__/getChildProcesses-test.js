...
function _getChildProcesses() {
  const data = require("../lib/getChildProcesses");
...
>>>>>>> Update
    ...
    it('converts to tree', async () => {
      await (async () => {
        ...
        const actual = await (0, _getChildProcesses().queryPs)('command').map(_getChildProcesses().childProcessTree).toPromise();
        ...
        function check(node) {
          let count = 1;
          const expected = (0, _nullthrows().default)(expectedMap.get(node.pid));
        ...
        const count = check((0, _nullthrows().default)(actual));
...
describe('getChildProcesses', () => {
  ...
  describe('durationInSeconds', () => {
    it('handles integers', () => {
      expect((0, _getChildProcesses().durationInSeconds)('5')).toBe(5);
    });
    it('handles doubles', () => {
      expect((0, _getChildProcesses().durationInSeconds)('2.5')).toBe(2.5);
    });
    it('handles minutes', () => {
      expect((0, _getChildProcesses().durationInSeconds)('5:00')).toBe(5 * 60);
    });
    it('handles hours', () => {
      expect((0, _getChildProcesses().durationInSeconds)('5:00:00')).toBe(5 * 60 * 60);
    });
    it('handles everything at once', () => {
      expect((0, _getChildProcesses().durationInSeconds)('500:10:20.5')).toBe(500 * 60 * 60 + 10 * 60 + 20.5);
    });
  });
});