...
exports.FlowIDEConnectionWatcher = void 0;
 ...
 * @format
 */
const defaultIDEConnectionFactory = (proc, fileCache) => new (_FlowIDEConnection().FlowIDEConnection)(proc, fileCache); // ESLint thinks the comment at the end is whitespace and warns. Worse, the autofix removes the
// entire comment as well as the whitespace.
// eslint-disable-next-line semi-spacing


const IDE_CONNECTION_MAX_WAIT_MS = 20
/* min */
* 60
/* s/min */
* 1000
/* ms/s */
;
const IDE_CONNECTION_MIN_INTERVAL_MS = 1000; // If a connection lives shorter than this, it is considered unhealthy (it probably crashed
// immediately for whatever reason)

const IDE_CONNECTION_HEALTHY_THRESHOLD_MS = 10 * 1000; // If we get this many unhealthy connections in a row, give up.
...
class FlowIDEConnectionWatcher {
  constructor(processFactory, fileCache, ideConnectionCallback, // Can be injected for testing purposes
  ...
  async _makeIDEConnection() {
    ...
    while (true) {
      const attemptStartTime = this._getTimeMS(); // Start the process. Eventually we should cancel by unsubscribing, but for now we'll just
      ...
      processStream.connect(); // eslint-disable-next-line no-await-in-loop

      proc = await processPromise; // dispose() could have been called while we were waiting for the above promise to resolve.
      ...
      if (proc != null || attemptEndTime > endTimeMS) {
        ...
        if (additionalWaitTime > 0) {
          (0, _log4js().getLogger)('nuclide-flow-rpc').info(`Waiting an additional ${additionalWaitTime} ms before retrying`); // eslint-disable-next-line no-await-in-loop
