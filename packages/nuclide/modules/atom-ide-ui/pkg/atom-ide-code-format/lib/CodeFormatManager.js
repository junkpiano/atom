      ...
      !formatRange.isEqual(buffer.getRange()) || fileProviders.length === 0)) {
        return _RxMin.Observable.defer(() => this._reportBusy(editor, Promise.all(rangeProviders.map(p => p.formatCode(editor, formatRange))))).switchMap(allEdits => {
          ...
          if (firstNonEmpty == null) {
            ...
            return _RxMin.Observable.of(firstNonEmpty);
          }
        });
      } else if (fileProviders.length > 0) {
        return _RxMin.Observable.defer(() => this._reportBusy(editor, Promise.all(fileProviders.map(p => p.formatEntireFile(editor, formatRange))))).switchMap(allResults => {
          const firstNonNull = allResults.find(result => result != null);

          if (firstNonNull == null) {
            return _RxMin.Observable.empty();
          } else {
            return _RxMin.Observable.of(firstNonNull);
          }
        }).map(({
          _,
          formatted
...
<<<<<<< HEAD
      ...
      }).flatMap(edits => _RxMin.Observable.of(...edits));
    } else if ((0, _config().getFormatOnSave)(editor)) {
      return this._formatCodeInTextEditor(editor, editor.getBuffer().getRange()).flatMap(edits => _RxMin.Observable.of(...edits));
    }
